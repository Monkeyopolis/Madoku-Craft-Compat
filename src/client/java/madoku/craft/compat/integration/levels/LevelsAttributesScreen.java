...
public final class LevelsAttributesScreen extends Screen {
	private static final Identifier BACKGROUND_TEXTURE_ATTRIBUTES =
		Identifier.fromNamespaceAndPath("madoku-craft-compat", "textures/containers/player_levels_attributes.png");
	private static final Identifier BACKGROUND_TEXTURE_VANILLA =
		Identifier.fromNamespaceAndPath("madoku-craft-compat", "textures/containers/player_levels_vanilla.png");
	private static final Identifier ENTRY_TEXTURE =
		Identifier.fromNamespaceAndPath("madoku-craft-compat", "textures/rows/player_entries.png");
	...
	protected void init() {
		this.addRenderableOnly((guiGraphics, mouseX, mouseY, partialTick) -> {
			...
			int panelX = panelX();
			int panelY = panelY();
			LevelsAttributesClientState.Snapshot snapshot = LevelsAttributesClientState.snapshot();
			...
			List<CompatLevelStat> stats = snapshot.visibleStats();
			...
			drawScaledCenteredText(guiGraphics, snapshot.username(), this.width / 2, usernameY, HEADER_TEXT_SCALE, 0xFF404040);
			...
			for (int index = 0; index < visibleEntries; index++) {
				CompatLevelStat stat = stats.get(index);
				int row = index / 2;
				int column = index % 2;
	...
	public void tick() {
		super.tick();
		if (stateVersion != LevelsAttributesClientState.version()) {
			rebuildWidgets();
		}
	}

	private void rebuildStatButtons() {
		stateVersion = LevelsAttributesClientState.version();
		LevelsAttributesClientState.Snapshot snapshot = LevelsAttributesClientState.snapshot();
		int panelX = panelX();
		int entriesTop = entriesTop();
		List<CompatLevelStat> stats = snapshot.visibleStats();
		...
		for (int index = 0; index < visibleEntries; index++) {
			...
			Button button = Button.builder(Component.literal("+"), pressed -> LevelsAttributesClient.requestStatUpgrade(stat))
	...
	private Identifier backgroundTexture(LevelsAttributesClientState.Snapshot snapshot) {
	...
	private void drawScaledLeftText(Object guiGraphics, String text, int x, int y, float scale, int color) {
		...
		net.minecraft.client.gui.GuiGraphicsExtractor graphics = (net.minecraft.client.gui.GuiGraphicsExtractor) guiGraphics;
		graphics.pose().pushMatrix();
		graphics.pose().scale(scale, scale);
		graphics.text(this.font, text, Math.round(x / scale), Math.round(y / scale), color, false);
